<template>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer d-flex">
            ...
            <button type="button" class="btn custom-register-btn" @click="registerGoal">등록</button>
          </div>
        </div>
      </div>
    </div>
  ...
  const registerGoal = () => {
    if (newGoal.value.title && newGoal.value.totalAmount) {
  ...
  .custom-register-btn {
  background-color: #ff007f; /* 등록 버튼의 배경색 설정 */
  border-color: #ff007f; /* 등록 버튼의 테두리 색 설정 */
  color: white; /* 글자색 설정 */
  }
  .custom-register-btn:hover {
  background-color: #e60072; /* Hover 시 조금 어두운 색 */
  border-color: #e60072;
  }
